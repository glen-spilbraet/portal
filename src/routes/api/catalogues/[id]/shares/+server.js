...
export async function POST({ params, request, cookies, platform }) {
	...
	await shareCatalogueWith(db, params.id, shared_with_email.trim().toLowerCase(), email);
	return json({ ok: true });
}
