...
import { renamePlanogramFolder, deletePlanogramFolder } from '$lib/db.js';
...
export async function PATCH({ params, request, cookies, platform }) {
	...
	await renamePlanogramFolder(db, params.id, name.trim());
	return json({ ok: true });
}
...
export async function DELETE({ params, cookies, platform }) {
	...
	await deletePlanogramFolder(db, params.id);
	return json({ ok: true });
}
